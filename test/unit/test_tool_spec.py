"""
Unit tests for toolguard.tool_spec.

Pins the derived views against the literal sets they replaced (TOO-45
punch-list #10) so a future registry edit that silently drops a tool fails
loudly, checks payload-key resolution end to end from the registry, and
guards the registry's own integrity (no duplicate names).
"""
...
class TestDerivedViewsPinnedToLiteralSets(unittest.TestCase):
    """Each derived view must equal what the hand-written set contained before."""

    def test_builtin_tools_matches_prior_literal_set(self):
        """Given the registry, when derived, then it equals the prior GOVERNED_TOOLS."""
        self.assertEqual(BUILTIN_TOOLS, {"Bash", "Read", "Write", "Edit"})

    def test_file_kind_tools_matches_prior_literal_set(self):
        """Given the registry, when derived, then it equals the prior FILE_TOOLS."""
        self.assertEqual(FILE_KIND_TOOLS, {"Read", "Write", "Edit"})

    def test_known_tool_names_matches_prior_literal_set(self):
        """Given the registry, when derived, then it equals the prior KNOWN_SUPPORTED_TOOLS."""
        self.assertEqual(
            KNOWN_TOOL_NAMES,
            {
                "Bash",
    ...
    def test_constants_module_constants_are_the_same_frozensets(self):
        ...
        Then they are the SAME object, not a second, independently-built copy
            (pins the registry's snapshot semantics being uniform everywhere).
        """
...
class TestDefaultGovernedTools(unittest.TestCase):
    ...
    def test_default_governed_tools_is_bash_read_write_edit_in_registry_order(self):
        ...
        Then it is exactly ('Bash', 'Read', 'Write', 'Edit'), in that order --
            a future registry edit that silently narrows the built-in set fails
            this test loudly.
        """
...
class TestRegistryIntegrity(unittest.TestCase):
    """The registry itself: no duplicate names, every entry fully resolvable."""

    def test_tools_by_name_has_one_entry_per_registry_entry(self):
        """
        Given the real _REGISTRY
        When indexed into TOOLS_BY_NAME
        Then the count matches exactly (a duplicated name would collapse silently
            in a plain dict comprehension -- this catches that).
        """
