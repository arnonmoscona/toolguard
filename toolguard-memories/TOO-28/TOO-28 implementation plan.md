...
- ~~The reason-text `.replace()` in `permission_resolution._reason_naming_real_group`~~ — **now TOO-79**, *"Compose the decision reason once, at the edge, from facts gathered along the way"*. Arnon's diagnosis, and it is better than the one recorded here: the prose is **constructed prematurely**, so every layer that later learns another fact patches the string it was handed. Four accretion sites, not one. It generalizes to any future enrichment that *characterizes* a rule rather than merely *filtering* it — which is why `program_source` needed no prose surgery and `auto_mode_behavior` did. The ticket also carries a second finding: `architecture_fitness.py`'s R3 predicate, whose whole job is catching reason-string parsing, missed this because `replace` is absent from `_REASON_STRING_METHODS` — and `replace` is the worst member to omit, since a stale one returns the original string, unchanged and plausible.
