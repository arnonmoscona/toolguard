...
title: 44-briefing
type: note
permalink: toolguard/too-45/reports/surprise/44-briefing
---

# File inventory


## toolguard/ — 77 modules
...
- `toolguard/env_config.py` (192 lines) — Environment-variable and ``.env`` configuration for toolguard: where the
- `toolguard/error_log.py` (184 lines) — Error, warning, conflict, and crash logging for toolguard.
...
- `toolguard/hook.py` (1372 lines) — Claude Code PreToolUse hook entry point: reads one hook event on stdin and
...
- `toolguard/normalization.py` (209 lines) — Path normalization for consistent pattern matching.
- `toolguard/once_per.py` (192 lines) — Facade for throttling a warning or an action to at most once per period, per
- `toolguard/once_per_store.py` (481 lines) — Cross-project ``(project, kind, scope)`` claim store backing "once per
...
- `toolguard/path_utils.py` (314 lines) — Low-level filesystem path helpers: the bounded walk up the parent directories,
...
- `toolguard/session_start.py` (484 lines) — Toolguard SessionStart Hook for Claude Code.
...
- `toolguard/tools/installer.py` (2337 lines) — ``toolguard-install``: an AGENT-FACING installer helper for toolguard.
...
- `toolguard/tools/transcript_harvest.py` (345 lines) — Transcript harvester: parse Claude Code conversation transcripts into the same
...
- `tools/architecture_fitness.py` (4005 lines) — Dev instrument for the TOO-45 architecture-refactoring loop: five modes over this
...
- `test/unit/test_architecture.py` (648 lines) — Architectural invariant tests for toolguard's module layering: governed modules
- `test/unit/test_architecture_fitness.py` (4103 lines) — Unit tests for ``tools/architecture_fitness.py``.
...
- `test/unit/test_env_config.py` (1051 lines) — Unit tests for toolguard environment configuration.
- `test/unit/test_error_log.py` (276 lines) — Unit tests for toolguard.error_log.
...
- `test/unit/test_hook.py` (3396 lines) — Unit tests for toolguard.hook: the PreToolUse entry point for Bash and file-path tools.
- `test/unit/test_hook_error_reporter.py` (521 lines) — Unit tests for the error-reporter wiring in toolguard.hook: main() owns one
...
packages = ["constants", "issues", "path_utils", "normalization", "patterns", "toml_scan", "_git", "install_provenance", "install_update", "file_lock", "tool_spec"]
