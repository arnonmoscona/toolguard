...
title: 78-briefing
type: note
permalink: toolguard/too-45/reports/surprise/78-briefing
---
...
- `toolguard/ambient.py` (112 lines) — The machine state toolguard reads: home directory, current directory, process
- `toolguard/api.py` (107 lines) — Public decision interface for toolguard.
- `toolguard/auto_migrate.py` (161 lines) — Unattended migration of divergent permissions from settings.local.json into
- `toolguard/compound.py` (1102 lines) — Compound command permission checking for toolguard.
- `toolguard/config.py` (2177 lines) — Configuration loading for toolguard.
- `toolguard/config_divergence.py` (238 lines) — Config divergence detection: permissions present in Claude's native
- `toolguard/config_types.py` (790 lines) — Plain configuration data types, split out of :mod:`toolguard.config` so this module can
...
- `toolguard/env_config.py` (198 lines) — Environment-variable and ``.env`` configuration for toolguard: where the
...
- `toolguard/install_provenance.py` (359 lines) — Detects whether the toolguard package currently governing this process might
...
- `toolguard/parser/bash_parser.py` (6976 lines) — (no module docstring)
- `toolguard/parser/command_extractor.py` (971 lines) — Split a bash command line into the leaf commands a permission rule can match.
- `toolguard/parser/command_model.py` (780 lines) — Abstract Command Model (IR) for the bash command extractor.
- `toolguard/parser/multiline.py` (579 lines) — The lexical pre-pass in front of the bash PEG parser.
- `toolguard/path_utils.py` (354 lines) — Low-level filesystem path helpers: anchoring a relative path, expanding a
...
- `toolguard/permission_resolution.py` (453 lines) — The decision engine for permission resolution.
- `toolguard/permissions.py` (486 lines) — Command string matching and permission decisions for the command-kind tools. The two
- `toolguard/resolve.py` (350 lines) — Permission resolver layer for toolguard.
...
- `toolguard/tools/decision_ledger.py` (376 lines) — Prior-decision ledger for the maintenance skill.
...
- `tools/architecture_fitness.py` (4513 lines) — Dev instrument for the TOO-45 architecture-refactoring loop, over this repo's own
...
- `test/unit/test_ambient.py` (351 lines) — Unit tests for toolguard.ambient: where toolguard reads home, cwd and the
- `test/unit/test_api.py` (1072 lines) — Unit tests for toolguard.api: the side-effect-free decide() primitive.
- `test/unit/test_architecture.py` (734 lines) — Architectural invariant tests for toolguard's module layering: governed modules
- `test/unit/test_architecture_fitness.py` (4509 lines) — Unit tests for ``tools/architecture_fitness.py``.
- `test/unit/test_ask_resolution.py` (408 lines) — Resolver tests for the ``ask`` permission list, driving the real decision engine
- `test/unit/test_assignment_prefix.py` (490 lines) — A leading ``NAME=value`` assignment must not hide the command under it from a deny
...
- `test/unit/test_env_config.py` (1128 lines) — Unit tests for toolguard environment configuration.
...
- `test/unit/test_hook.py` (3429 lines) — Unit tests for toolguard.hook: the PreToolUse entry point for Bash and file-path tools.
...
- `test/unit/test_path_utils.py` (70 lines) — Unit tests for toolguard.path_utils: the path helpers that answer against
...
- `test/unit/test_tools_maintenance.py` (1858 lines) — Unit tests for the maintenance aggregator (toolguard.tools.maintenance).
