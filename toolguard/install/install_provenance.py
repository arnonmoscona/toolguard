...
# toolguard.install._git.run_git.
import subprocess
import tomllib
...
from toolguard.install._git import run_git
from toolguard.foundation.constants import DIST_NAME as _DEFAULT_NAME
...
def governing_package_root() -> Path:
    ...
    Derived from the package rather than from this module's own ``__file__``:
    this module sits in a layer subpackage, so its parent is not the root.
    """
    return Path(toolguard.__file__).resolve().parent
