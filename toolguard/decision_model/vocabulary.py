"""
Plain configuration data types, split out of :mod:`toolguard.configuration.config` so this module can
stay a leaf.

It imports only :mod:`toolguard.decision_model.rule_entry`, and must never import :mod:`toolguard.configuration.config`
...
class ToolPatternLayer:
    ...
    The wrapper-intact :class:`~toolguard.decision_model.rule_entry.RuleEntry` tuples (``allow_entries``/
...
def entry_for_pattern(
    ...
    """
    Find the :class:`~toolguard.decision_model.rule_entry.RuleEntry` behind a matched pattern.
    ...
    Returns:
        The :class:`~toolguard.decision_model.rule_entry.RuleEntry` behind the first layer
...
class CommandSpellings:
    ...
    :mod:`toolguard.engine.permissions`, whose per-module import allow-list in
...
class LevelMatch:
    ...
    Lives here rather than in :mod:`toolguard.engine.resolve`: :mod:`toolguard.engine.permissions` and
    :mod:`toolguard.engine.file_matching` both construct it, and ``resolve.py`` imports FROM both of
    ...
    Attributes:
        ...
        reason: Human-readable reason for THIS level's/pool's own match,
            ...
            :func:`~toolguard.engine.permission_resolution._resolve_unclamped`).
        ...
        matched_entry_kind: The rule's ACTUAL group (``'allow'``/``'deny'``/``'ask'``)
            ...
            ``decision``: :func:`~toolguard.engine.permission_resolution._matched_rule_lookup`
            ...
            ``None`` when unset -- :mod:`~toolguard.engine.permissions`/
            :mod:`~toolguard.engine.file_matching` never set it; it is filled in by
            :mod:`~toolguard.engine.permission_resolution`'s own callers, which alone know
...
class UnitVerdict:
    ...
    Lives here rather than in :mod:`toolguard.engine.resolve` (where it is also constructed):
    :mod:`toolguard.engine.compound` constructs it too, and ``resolve.py`` imports FROM
    ...
    Attributes:
        sub_command: The sub-command's real, full source text. For an undecidable segment
            ...
            outer-command stub :mod:`toolguard.engine.compound` resolves internally to check for an
        ...
        fallback_cause: ``'no_match'``, ``'undecidable'``, or ``None`` -- WHY a fallback
            ...
            in :mod:`toolguard.engine.resolve`/:mod:`toolguard.engine.compound`; never derived downstream.
        ...
        audit_only: ``True`` for a raw record itemising one of an ``'inline_code'``
            ...
            :mod:`toolguard.engine.compound`) -- present in ``RuntimeVerdict.sub_matches``
            ...
            :func:`~toolguard.engine.resolve._deciding_sub_match`'s search for the
...
class RuntimeVerdict:
    ...
    ---------------------------------------------------
    :mod:`toolguard.engine.permission_resolution` constructs this type directly, as the internal
    cascade verdict, and its own docstring declares it never imports
    :mod:`toolguard.engine.resolve` -- correctly, since ``resolve.py`` imports FROM
    ``permission_resolution``, so the reverse would be circular.
    :mod:`toolguard.decision_model.vocabulary` is the shared leaf both modules can import -- and since
    ...
    ``tool``/``target``
    --------------------
    ``None`` until populated by one of :mod:`toolguard.engine.resolve`'s two public entry points
    (:func:`~toolguard.engine.resolve.resolve_bash_permission_detailed`,
    :func:`~toolguard.engine.resolve.resolve_file_path_permission_detailed`), the only callers with
    ...
    ``'Bash'``, matching :func:`~toolguard.engine.resolve.resolve_bash_permission_detailed`'s
    ...
    Attributes:
        ...
        provenance: Provenance of the winning rule (for a compound Bash
            ...
            :func:`~toolguard.engine.resolve._deciding_sub_match`), or ``None`` for a
        ...
        sub_matches: One :class:`UnitVerdict` per audited unit, in order -- one per
            ...
            :attr:`~toolguard.engine.compound.CommandUnit.audits_as_one`). Empty for a
        ...
        additional_context: The accumulated ``additionalContext`` enrichment
            ...
            :func:`toolguard.engine.compound.cap_context_words` regardless of
        ...
        matched_rule: The deciding rule's pattern text (wrapper-free), or
            ...
            pattern here -- see :func:`~toolguard.engine.resolve.resolve_file_path_permission_detailed`'s
        ...
        fallback_cause: The counterpart of :attr:`UnitVerdict.fallback_cause` at the RUNTIME
            ...
            allowed unit) -- see :func:`~toolguard.engine.compound.combine_strictest`. ``None``
...
# toolguard.configuration.config, so config cannot be typed as Configuration at its entry
...
# pyright without adding an import edge to toolguard.configuration.config.


class ResolutionConfig(Protocol):
    """
    The minimal configuration surface :mod:`toolguard.engine.permission_resolution` reads from
    ``config`` to run its decision cascade.

    Closes the gap left by :mod:`toolguard.engine.permission_resolution` being architecturally
    forbidden from importing :mod:`toolguard.configuration.config`: any object handed in as ``config`` --
    in practice always a real :class:`~toolguard.configuration.config.Configuration`, but a test double
    ...
    :mod:`toolguard.engine.resolve` needs a wider surface, so none of its ``config`` parameters
    ...
    def parse_failures(self) -> Tuple[Tuple[Path, str], ...]:
        ...
        :func:`~toolguard.engine.permission_resolution.apply_parse_failure_floor` to clamp a
        ...
        :class:`~toolguard.configuration.config.Configuration`, is a frozen dataclass, and pyright treats
    ...
    def permission_levels_with_provenance(
        ...
        matched by :func:`~toolguard.engine.permissions.decide_command_at_level_detailed` or
        :func:`~toolguard.engine.file_matching.decide_file_path_at_level_detailed`, and ``layers``
...
class ResolveConfig(ResolutionConfig, Protocol):
    """
    The configuration surface :mod:`toolguard.engine.resolve` itself needs -- a strict superset of
    ...
    :func:`~toolguard.engine.permission_resolution.resolve_command_permission`/
    :func:`~toolguard.engine.permission_resolution.resolve_file_path_permission` stays sound
    without a cast.
    """
    ...
    def hard_deny(self, tool_name: str) -> Tuple[Tuple[str, ...], Tuple[str, ...]]:
        ...
        :func:`~toolguard.engine.resolve.resolve_bash_permission_detailed` as a plain argument.
        """
        ...
    ...
    def hard_deny_entries(
        ...
        as wrapper-intact :class:`~toolguard.decision_model.rule_entry.RuleEntry` objects (carrying
    ...
    def resolved_undecidable_fallback(self) -> str:
        ...
        Passed to :func:`~toolguard.engine.compound.judge_unit` for every unit of a compound Bash
        ...
        :func:`~toolguard.engine.permission_resolution.resolve_command_permission` at all), and an
...
class PathAnchoring(Protocol):
    ...
    as a whole needs more -- :func:`~toolguard.engine.file_matching.check_file_path_hard_deny` is
...
class FilePathResolutionConfig(ResolutionConfig, PathAnchoring, Protocol):
    """
    The configuration surface :func:`~toolguard.engine.permission_resolution.resolve_file_path_permission`
    ...
    layer down into :func:`~toolguard.engine.file_matching.decide_file_path_at_level_detailed`
...
# Configuration". A concrete :class:`~toolguard.foundation.invocation.Invocation`
...
class ResolutionContext(Protocol):
    """
    What :func:`~toolguard.engine.permission_resolution.resolve_command_permission` needs about one
...
class ResolveContext(ResolutionContext, Protocol):
    ...
    :func:`~toolguard.engine.file_matching.check_file_path_hard_deny`: ``config`` must additionally
...
class FilePathResolutionContext(ResolutionContext, Protocol):
    """
    As :class:`ResolutionContext`, narrowed for
    :func:`~toolguard.engine.permission_resolution.resolve_file_path_permission`: ``config`` must
