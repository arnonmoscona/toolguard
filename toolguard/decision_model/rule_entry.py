...
:class:`~toolguard.foundation.issues.Issue` explaining why.

This module imports nothing from :mod:`toolguard` except :mod:`toolguard.foundation.issues` and
:mod:`toolguard.foundation.constants` (both leaves themselves), keeping it a leaf that both
:mod:`toolguard.configuration.config` and :mod:`toolguard.configuration.config_validation` can depend on without a
...
:func:`toolguard.configuration.config._multiline_structured_entry_diagnostic`.
"""
...
#: "override": see :mod:`toolguard.engine.permission_resolution`'s own docstring for
...
#: see :data:`~toolguard.foundation.constants.PROGRAM_SOURCE_NOT_FILE`.
...
class RuleEntry:
    ...
    #: :func:`~toolguard.configuration.config_write_guard.verified_write_config`'s
    ...
    def program_source(self) -> Optional[str]:
        ...
        pattern (:data:`~toolguard.foundation.constants.FILE_TOOLS`) -- the key never applies
        there, and this accessor is what
        :func:`~toolguard.engine.permission_resolution.resolve_command_permission`'s
        pre-match filtering (:func:`~toolguard.engine.permission_resolution._level_pattern_buckets`)
        reads to drop such an entry before it ever reaches the matcher.

        Returns:
            :data:`~toolguard.foundation.constants.PROGRAM_SOURCE_FILE` or
            :data:`~toolguard.foundation.constants.PROGRAM_SOURCE_NOT_FILE`, or ``None`` when this
...
def _additional_context_issues(pattern: str, metadata: Mapping[str, object]) -> tuple:
    ...
    Reported as an ``error``-level :class:`~toolguard.foundation.issues.Issue`, but
...
def _auto_mode_behavior_issues(pattern: str, metadata: Mapping[str, object]) -> tuple:
    ...
    Reported as an ``error``-level :class:`~toolguard.foundation.issues.Issue`, same shape as
...
def _program_source_issues(pattern: str, metadata: Mapping[str, object]) -> tuple:
    ...
    A ``Tool(...)`` naming a file-path tool (:data:`~toolguard.foundation.constants.FILE_TOOLS`)
