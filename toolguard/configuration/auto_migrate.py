...
from toolguard.foundation import once_per
...
from toolguard.observability.error_reporter import report_notice, report_warning
from toolguard.foundation.once_per import Repeat
from toolguard.foundation.path_utils import expanduser
from toolguard.configuration.permission_migration import MigrationOutcome, migrate
...
#: How long ``migrate()`` waits for this project's lock when called from here.
#: Sub-second deliberately: a contended lock should decline fast and wait for
#: tomorrow rather than hold up session start.
...
def run_auto_migration_for_config(project_root: Path, config: Configuration) -> bool:
    """
    Run :func:`run_auto_migration` for *config*, when it enables auto-migration.

    Owns the conversion of ``TakeoverConfig`` into the plain dict the migration
    workflow still consumes, so a caller supplies a ``Configuration`` and nothing
    outside this layer has to know that dict's shape.
    ...
    Returns:
        ...
        including auto-migration being disabled.
    """
    config_sync = config.config_sync_settings()
    if not config_sync["auto_migrate"]:
        return False
    takeover = config.takeover_mode()
    return run_auto_migration(
        project_root,
        dict(config_sync),
        {
...
def run_auto_migration(
    ...
    Side effects:
        ...
        outcome are reported via :mod:`toolguard.observability.error_reporter`.
    """
    ...
    backup_dir = expanduser(config.resolve_config_path(backup_dir_str))
