...
from toolguard.configuration.config_types import ConfigLayer as ConfigLayer
from toolguard.decision_model.vocabulary import ConflictOverride as ConflictOverride
from toolguard.decision_model.vocabulary import Provenance as Provenance
from toolguard.decision_model.vocabulary import RuntimeVerdict as RuntimeVerdict
from toolguard.configuration.config_types import TakeoverConfig as TakeoverConfig
from toolguard.configuration.config_types import (
    TakeoverEnabledConflict as TakeoverEnabledConflict,
)
from toolguard.decision_model.vocabulary import ToolPatternLayer as ToolPatternLayer
from toolguard.configuration.config_types import (
...
from toolguard.observability.error_reporter import report_warning
from toolguard.foundation.issues import Issue
from toolguard.foundation.path_utils import require_project_root
from toolguard.decision_model.rule_entry import (
    RuleEntry,
    entries_for_tool,
    normalize_entry,
)
from toolguard.decision_model.rule_entry import is_tool_wrapper as is_tool_wrapper
from toolguard.decision_model.rule_entry import normalize_entries_preserving
from toolguard.foundation.tool_spec import DEFAULT_GOVERNED_TOOLS
from toolguard.foundation.toml_scan import find_multiline_structured_entry_line
...
def wrap_tool_pattern(tool: str, body: str) -> str:
    ...
    The structural inverse of :func:`~toolguard.decision_model.rule_entry._strip_tool_wrapper`: given a tool name and a
...
class Configuration:
    ...
    :mod:`toolguard.engine.permission_resolution`, fed by
    ...
    Attributes:
        ...
        parse_failures: ``(path, message)`` pairs for every governed config
            ...
            :func:`toolguard.configuration.config_validation.find_wrong_shaped_permission_lists`).
            ...
            (see :func:`~toolguard.engine.permission_resolution._apply_ask_floor`)
    ...
    def governed_tools(self) -> Tuple[str, ...]:
        ...
        Defaults to :data:`~toolguard.foundation.tool_spec.DEFAULT_GOVERNED_TOOLS`
    ...
    def assignments_looked_past_when_granting(self) -> Tuple[str, ...]:
        ...
        prefix regardless -- see :func:`toolguard.engine.permissions.match_spellings`.
    ...
    def _extract_tool_entries(
        ...
        :func:`toolguard.decision_model.rule_entry.normalize_entry` (plain string or
    ...
    def resolved_undecidable_fallback(self) -> str:
        ...
        :func:`toolguard.engine.compound._apply_undecidable_floor`.
    ...
    def unrecognized_fallback_settings(
        ...
        through :class:`~toolguard.decision_model.vocabulary.TakeoverConfig`, which does not retain
        per-layer provenance for that field, so it cannot name the file the way this
        warning must.

        Returns:
            Tuple of :class:`~toolguard.decision_model.vocabulary.UnrecognizedFallbackSetting`, in
    ...
    def toolguard_permissions(self) -> Mapping:
        ...
        :func:`toolguard.decision_model.rule_entry.normalize_entries_preserving` rather than
        ...
        Returns:
            ...
            :class:`~toolguard.decision_model.rule_entry.RuleEntry`.
        """
        ...
        return tuple(
            Issue(
                ...
                corrective_steps=(
                    ...
                    "toolguard.engine.permission_resolution.resolve_permission_cascade) "
    ...
    def _unrecognized_fallback_setting_issues(self) -> Tuple[Issue, ...]:
        ...
        :attr:`~toolguard.decision_model.vocabulary.UnrecognizedFallbackSetting.falls_back_to`), but
    ...
    def _hard_deny_layer_issues(self) -> Tuple[Issue, ...]:
        ...
        :func:`~toolguard.configuration.config_validation.find_hard_deny_entry_issues`.
        ``[hard_deny]`` is never merged across layers (see
        :data:`~toolguard.configuration.config_validation._SHAPE_CHECKED_SECTIONS`), so
    ...
    def _regex_control_char_issues(self) -> Tuple[Issue, ...]:
        ...
        :func:`~toolguard.configuration.config_validation.find_regex_control_char_issues`.
    ...
    def _merged_permission_issues(self) -> Tuple[Issue, ...]:
        ...
        :func:`~toolguard.configuration.config_validation.validate_permissions`. A layer
...
def _multiline_structured_entry_diagnostic(
    ...
    :func:`toolguard.foundation.toml_scan.find_multiline_structured_entry_line`, which
