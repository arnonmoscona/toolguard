...
imported (:mod:`toolguard.configuration.auto_migrate`, :mod:`toolguard.tools.installer`,
...
from toolguard.foundation import ambient, file_lock
...
from toolguard.configuration.config_validation import extract_tool_name
...
#: lock_timeout_seconds explicitly; see toolguard.configuration.auto_migrate.
...
def _migration_lock_path(project_root: Path) -> Path:
    ...
    :func:`toolguard.foundation.once_per_store._project_key` (``str(project)``, no
...
def generate_permissions_section(
    ...
    Args:
        permissions: Dictionary with 'allow', 'deny', 'ask' keys, each a list
            of pattern ``str`` and/or :class:`~toolguard.decision_model.rule_entry.RuleEntry`
            ...
            :func:`~toolguard.configuration.rule_sort.render_toml_entry`).
...
def _patterns_from_permissions(
    ...
    :func:`~toolguard.configuration.config_write_guard.verified_write_config`'s content-loss
    ...
    Delegates to :func:`~toolguard.decision_model.rule_entry.real_patterns` (TOO-19 review
    fix) rather than extracting ``.pattern`` unconditionally: an entry from
    :func:`~toolguard.decision_model.rule_entry.normalize_entries_preserving` may carry a
...
def write_toml_config(
    ...
    :func:`~toolguard.configuration.config_write_guard.verified_write_config`, never a raw
...
def write_json_config(
    ...
    Writes via :func:`~toolguard.configuration.config_write_guard.verified_write_config`,
...
def migrate(
    ...
    that :mod:`toolguard.configuration.auto_migrate` calls directly for unattended
