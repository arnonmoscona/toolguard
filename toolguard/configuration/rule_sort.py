...
or a :class:`~toolguard.decision_model.rule_entry.RuleEntry`; a raw ``dict`` is never
accepted, so a caller holding un-normalized config data normalizes it first
(:func:`~toolguard.decision_model.rule_entry.normalize_entries_preserving`).
...
from toolguard.decision_model.rule_entry import PATTERN_KEY, RuleEntry
from toolguard.foundation.toml_scan import (  # noqa: F401 -- re-exported
    ArrayElement,
    locate_subsection,
...
def render_toml_entry(entry: RuleEntryOrStr) -> str:
    ...
    Args:
        entry: A bare pattern ``str``, or a :class:`RuleEntry` rendered from
            its :meth:`~toolguard.decision_model.rule_entry.RuleEntry.to_source` -- which
...
def _rule_pattern_of_value(value: object) -> str:
    ...
    lives under :data:`toolguard.decision_model.rule_entry.PATTERN_KEY` (``"match"``), so
    ...
    :func:`~toolguard.decision_model.rule_entry.normalize_entry`'s job, not this
...
def subsection_line_range(
    ...
    """
    location = locate_subsection(section_text, perm_type)
    if location is None:
        return None
...
def reassemble_permissions_section(
    ...
    :func:`~toolguard.decision_model.rule_entry.merge_entries` deliberately preserves such a
    ...
    Args:
        ...
        new_permissions: New permission values to use, as a dict with keys
            ...
            ``str`` and/or :class:`~toolguard.decision_model.rule_entry.RuleEntry`. A
