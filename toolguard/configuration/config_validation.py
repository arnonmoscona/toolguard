...
def find_wrong_shaped_permission_lists(
    ...
    :meth:`~toolguard.configuration.config.Configuration._extract_tool_entries`, which is
    ...
    result into :attr:`~toolguard.configuration.config.Configuration.parse_failures` so the
...
def validate_permissions(config: dict) -> Tuple[Issue, ...]:
    ...
    :func:`toolguard.decision_model.rule_entry.normalize_entry` has to say about it -- an
    ...
    Returns:
        A tuple of :class:`toolguard.foundation.issues.Issue`. Issues sharing a
...
def find_regex_control_char_issues(content: Mapping[str, object]) -> Tuple[Issue, ...]:
    ...
    Checks :func:`~toolguard.foundation.patterns.parse_pattern`'s already-stripped body,
