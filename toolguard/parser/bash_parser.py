...
class TreeNode50(TreeNode):
    def __init__(self, text, offset, elements):
        ...
        self.assignment_prefix = elements[0]
        self.command_start = elements[1]
...
class TreeNode52(TreeNode):
    def __init__(self, text, offset, elements):
        ...
        self.assignment_name = elements[0]
        self.assignment_value = elements[2]
        self.spacing = elements[3]
...
class Grammar(object):
    ...
    def _read_program(self):
        ...
        if cached:
            ...
            return cached[0]
        index1 = self._offset
        address0 = self._read_comment_only_program()
        if address0 is FAILURE:
            self._offset = index1
            address0 = self._read_command_program()
            if address0 is FAILURE:
                self._offset = index1
        self._cache["program"][index0] = (address0, self._offset)
        return address0

    def _read_command_program(self):
        address0, index0 = FAILURE, self._offset
        cached = self._cache["command_program"].get(index0)
        if cached:
            self._offset = cached[1]
            return cached[0]
        ...
        if elements0 is None:
            ...
            self._offset = self._offset
        self._cache["command_program"][index0] = (address0, self._offset)
        return address0

    def _read_comment_only_program(self):
        address0, index0 = FAILURE, self._offset
        cached = self._cache["comment_only_program"].get(index0)
        if cached:
            self._offset = cached[1]
            return cached[0]
        index1, elements0 = self._offset, []
        ...
        while True:
            address2 = self._read_line_ws_char()
            if address2 is not FAILURE:
            ...
            if self._offset < self._input_size:
                ...
                if self._offset == self._failure:
                    self._expected.append(
                        ("BashParser::comment_only_program", "<any char>")
                    )
            self._offset = index3
        ...
        if elements0 is None:
            ...
            self._offset = self._offset
        self._cache["comment_only_program"][index0] = (address0, self._offset)
        return address0
    ...
    def _read_command_name(self):
        ...
        if cached:
            ...
            return cached[0]
        index1 = self._offset
        address0 = self._read_assigned_command()
        if address0 is FAILURE:
            self._offset = index1
            address0 = self._read_command_start()
            if address0 is FAILURE:
                self._offset = index1
        self._cache["command_name"][index0] = (address0, self._offset)
        return address0

    def _read_assigned_command(self):
        address0, index0 = FAILURE, self._offset
        cached = self._cache["assigned_command"].get(index0)
        if cached:
            self._offset = cached[1]
            return cached[0]
        index1, elements0 = self._offset, []
        address1 = FAILURE
        address1 = self._read_assignment_prefix()
        if address1 is not FAILURE:
            elements0.append(address1)
            address2 = FAILURE
            address2 = self._read_command_start()
            if address2 is not FAILURE:
                elements0.append(address2)
            else:
                elements0 = None
        ...
        if elements0 is None:
            ...
            self._offset = self._offset
        self._cache["assigned_command"][index0] = (address0, self._offset)
        return address0

    def _read_assignment_prefix(self):
        address0, index0 = FAILURE, self._offset
        cached = self._cache["assignment_prefix"].get(index0)
        if cached:
            self._offset = cached[1]
            return cached[0]
        index1, elements0, address1 = self._offset, [], None
        while True:
            address1 = self._read_assignment()
            if address1 is not FAILURE:
                elements0.append(address1)
            else:
                break
        if len(elements0) >= 1:
            ...
            address0 = FAILURE
        self._cache["assignment_prefix"][index0] = (address0, self._offset)
        return address0

    def _read_command_start(self):
        address0, index0 = FAILURE, self._offset
        cached = self._cache["command_start"].get(index0)
        if cached:
            self._offset = cached[1]
            return cached[0]
        index1 = self._offset
        ...
        if address0 is FAILURE:
            self._offset = index1
            address0 = self._read_command_word()
            if address0 is FAILURE:
                self._offset = index1
        self._cache["command_start"][index0] = (address0, self._offset)
        return address0
    ...
    def _read_assignment(self):
        ...
        index1, elements0 = self._offset, []
        address1 = FAILURE
        address1 = self._read_assignment_name()
        if address1 is not FAILURE:
            elements0.append(address1)
            address2 = FAILURE
            index2 = self._offset
            ...
            if chunk0 == "+=":
                ...
                if self._offset > self._failure:
                    ...
                    self._expected = []
                if self._offset == self._failure:
                    self._expected.append(("BashParser::assignment", '"+="'))
            if address2 is FAILURE:
                self._offset = index2
                chunk1, max1 = None, self._offset + 1
                if max1 <= self._input_size:
            ...
            if address2 is not FAILURE:
                elements0.append(address2)
                address3 = FAILURE
                address3 = self._read_assignment_value()
                if address3 is not FAILURE:
                    elements0.append(address3)
                    address4 = FAILURE
                    address4 = self._read_spacing()
    ...
    def _read_assignment_name(self):
        address0, index0 = FAILURE, self._offset
        cached = self._cache["assignment_name"].get(index0)
        if cached:
            self._offset = cached[1]
            return cached[0]
        address0 = self._read_identifier()
        self._cache["assignment_name"][index0] = (address0, self._offset)
        return address0

    def _read_assignment_value(self):
        address0, index0 = FAILURE, self._offset
        cached = self._cache["assignment_value"].get(index0)
        if cached:
            self._offset = cached[1]
            return cached[0]
        index1, elements0, address1 = self._offset, [], None
        while True:
            index2 = self._offset
            address1 = self._read_quoted_string()
            if address1 is FAILURE:
                self._offset = index2
                address1 = self._read_escaped_char()
                if address1 is FAILURE:
                    self._offset = index2
                    address1 = self._read_tight_cmd_sub()
                    if address1 is FAILURE:
                        self._offset = index2
                        ...
                        if address1 is FAILURE:
                            ...
                            if elements1 is None:
                                ...
                                address1 = TreeNode(
                                    self._input[index3 : self._offset],
                                    index3,
                                    elements1,
                                )
                                self._offset = self._offset
        ...
        if len(elements0) >= 0:
            ...
            address0 = FAILURE
        self._cache["assignment_value"][index0] = (address0, self._offset)
        return address0

    def _read_tight_cmd_sub(self):
        address0, index0 = FAILURE, self._offset
        cached = self._cache["tight_cmd_sub"].get(index0)
        if cached:
            self._offset = cached[1]
            return cached[0]
        index1 = self._offset
        ...
        if chunk0 == "$(":
            ...
            if self._offset > self._failure:
                ...
                self._expected = []
            if self._offset == self._failure:
                self._expected.append(("BashParser::tight_cmd_sub", '"$("'))
        if address1 is not FAILURE:
            elements0.append(address1)
            address2 = FAILURE
            address2 = self._read_spacing()
        ...
        if elements0 is None:
            ...
            address0 = TreeNode53(self._input[index2 : self._offset], index2, elements0)
            self._offset = self._offset
        if address0 is FAILURE:
            self._offset = index1
            index3, elements1 = self._offset, []
            ...
            if address6 is not FAILURE:
                ...
                if address7 is not FAILURE:
                    elements1.append(address7)
                    address8 = FAILURE
                    address8 = self._read_compound_command()
                    if address8 is not FAILURE:
                        elements1.append(address8)
                        address9 = FAILURE
                        address9 = self._read_spacing()
                        if address9 is not FAILURE:
            ...
            if address0 is FAILURE:
                self._offset = index1
        self._cache["tight_cmd_sub"][index0] = (address0, self._offset)
        return address0
    ...
    def _read_var_ref(self):
        ...
        if elements0 is None:
            ...
            address0 = TreeNode70(self._input[index2 : self._offset], index2, elements0)
            self._offset = self._offset
        if address0 is FAILURE:
            self._offset = index1
            index3, elements1 = self._offset, []
