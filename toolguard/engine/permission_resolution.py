...
``Configuration`` (:mod:`toolguard.configuration.config`) is a query object over resolved config; this
...
passes through (see :mod:`toolguard.engine.resolve`): both Bash/MCP-terminal (per sub-command) and
file-path (Read/Write/Edit) resolution call :func:`resolve_command_permission`/
:func:`resolve_file_path_permission` respectively.

It never imports :mod:`toolguard.configuration.config` or :mod:`toolguard.engine.resolve` -- only
:mod:`toolguard.decision_model.vocabulary`, :mod:`toolguard.foundation.constants`, :mod:`toolguard.engine.permissions`,
:mod:`toolguard.engine.file_matching`, and the stdlib, and none of the toolguard ones import back into
...
at runtime this module and :mod:`toolguard.engine.file_matching` still call a real ``Configuration``'s
...
:class:`~toolguard.decision_model.vocabulary.ResolutionContext`; :func:`resolve_file_path_permission`
takes :class:`~toolguard.decision_model.vocabulary.FilePathResolutionContext`, the same surface narrowed
so ``context.config`` additionally supports ``resolve_config_path`` (project-root anchoring,
forwarded to :mod:`toolguard.engine.file_matching`) -- see those Protocols' own docstrings for what
each member means. A concrete :class:`~toolguard.foundation.invocation.Invocation` structurally
satisfies both; this module never imports it, the same way it never imports
:mod:`toolguard.configuration.config`.
:func:`~toolguard.decision_model.vocabulary.provenance_for_pattern`/
:func:`~toolguard.decision_model.vocabulary.entry_for_pattern` live in :mod:`toolguard.decision_model.vocabulary`,
beside :class:`~toolguard.decision_model.vocabulary.ToolPatternLayer`, and are imported and called
...
:func:`~toolguard.engine.permissions.decide_command_at_level_detailed`/
:func:`~toolguard.engine.file_matching.decide_file_path_at_level_detailed`) before
:func:`resolve_permission_cascade` folds the results -- a pure function of already-computed
:class:`~toolguard.decision_model.vocabulary.LevelMatch` values, taking no callable and no ``config`` at
...
from toolguard.decision_model.vocabulary import (
...
from toolguard.engine.permissions import decide_command_at_level_detailed
...
def apply_parse_failure_floor(
    ...
    (:func:`toolguard.engine.resolve.resolve_bash_permission_detailed`, which can
...
def _apply_ask_floor(
    ...
    to/from :class:`~toolguard.decision_model.vocabulary.RuntimeVerdict` and clearing the
...
def _reason_naming_real_group(result: LevelMatch) -> str:
    ...
    *result.reason*, with its "matches {decision} pattern" clause renamed to the
    ...
    :mod:`toolguard.engine.permissions`/:mod:`toolguard.engine.file_matching` build this exact
    ...
    :mod:`toolguard.engine.permissions` -- that makes the matcher know more about the final
...
def _matched_rule_lookup(layers: Tuple[ToolPatternLayer, ...], result: LevelMatch):
    ...
    own docstring restricts its toolguard imports to :mod:`toolguard.decision_model.vocabulary`,
    :mod:`toolguard.foundation.constants`, :mod:`toolguard.engine.permissions`, and
    :mod:`toolguard.engine.file_matching`, and neither ``Provenance`` nor ``RuleEntry`` is
    among them; see :func:`~toolguard.decision_model.vocabulary.provenance_for_pattern`/
    :func:`~toolguard.decision_model.vocabulary.entry_for_pattern`, which this delegates to, for
    the real types.
...
def _resolve_unclamped(
    ...
    Args:
        ...
        has_any_rules: Whether the tool has ANY rule configured anywhere --
            see :meth:`~toolguard.decision_model.vocabulary.ResolutionConfig.has_any_rules`.
        no_match_fallback: The effective ``no_match_fallback`` policy -- see
            :meth:`~toolguard.decision_model.vocabulary.ResolutionConfig.resolved_no_match_fallback`.
...
def resolve_permission_cascade(
    ...
    than in the Bash-specific compound pipeline (:mod:`toolguard.engine.compound`),
    ...
    Args:
        ...
        has_any_rules: See :meth:`~toolguard.decision_model.vocabulary.ResolutionConfig.has_any_rules`.
        no_match_fallback: See :meth:`~toolguard.decision_model.vocabulary.ResolutionConfig.resolved_no_match_fallback`.
...
def _effective_no_match_fallback(context: ResolutionContext) -> str:
    ...
    ``context.permission_mode`` is :data:`~toolguard.decision_model.vocabulary.AUTO_PERMISSION_MODE`,
...
def _level_pattern_buckets(
    ...
    Returns:
        ...
        :class:`~toolguard.decision_model.vocabulary.LevelMatch` with
...
def resolve_command_permission(
    ...
    *,
    ...
    :func:`~toolguard.engine.permissions.decide_command_at_level_detailed`, then
    ...
    Args:
        context: Supplies ``tool_name``, ``config``, and ``extended_syntax`` --
            see :class:`~toolguard.decision_model.vocabulary.ResolutionContext`.
...
def resolve_file_path_permission(
    ...
    :func:`~toolguard.engine.file_matching.decide_file_path_at_level_detailed`
    (which needs ``config`` for project-root anchoring -- see
    :class:`~toolguard.decision_model.vocabulary.FilePathResolutionConfig`), then folds
    the results with :func:`resolve_permission_cascade`. The production
    entry point for Read/Write/Edit resolution -- see
    :mod:`toolguard.engine.resolve`'s file-path resolver, called AFTER the
    unoverridable ``[hard_deny]`` pool check.

    Args:
        context: Supplies ``tool_name``, ``config``, and ``extended_syntax`` --
            see :class:`~toolguard.decision_model.vocabulary.FilePathResolutionContext`.
