...
from toolguard.decision_model.vocabulary import ConflictOverride
from toolguard.decision_model.vocabulary import LevelMatch as LevelMatch
from toolguard.decision_model.vocabulary import RuntimeVerdict as RuntimeVerdict
from toolguard.decision_model.vocabulary import UnitVerdict as UnitVerdict
...
def resolve_file_path_permission_detailed(
    ...
    :func:`~toolguard.engine.permission_resolution.resolve_file_path_permission`.
...
def _deciding_sub_match(
    ...
    :func:`toolguard.engine.compound.judge_unit`) is skipped entirely, for every
    *decision*: it never decided anything on its own, so it must never be
    ...
    Args:
        decision: The compound's final decision, after the parse-failure
            floor (:func:`~toolguard.engine.permission_resolution.apply_parse_failure_floor`)
...
def resolve_bash_permission_detailed(
    ...
    :func:`toolguard.engine.compound.combine_strictest`. Foreign inline code /
    ...
    :func:`toolguard.engine.compound.judge_unit`.
    ...
    if not units:
        ...
        # combine_strictest's own generic "No commands to evaluate"
        ...
        combined = combine_strictest(unit_verdicts)
