...
The public entry point, :func:`~toolguard.engine.resolve.resolve_file_path_permission_detailed`,
...
from toolguard.foundation.constants import DECISION_DENY
from toolguard.foundation.normalization import expand_tilde
from toolguard.foundation.patterns import PatternType, match_pattern, parse_pattern
from toolguard.engine.permissions import is_universal_pattern, resolve_allow_ask
...
def _anchor_file_pattern(
    ...
    Args:
        ...
        config: Provides project-root anchoring -- see
            :class:`~toolguard.decision_model.vocabulary.PathAnchoring`.
...
def decide_file_path_at_level_detailed(
    ...
    Mirrors :func:`toolguard.engine.permissions.decide_command_at_level_detailed` for
    ...
    Returns:
        A :class:`~toolguard.decision_model.vocabulary.LevelMatch` when this level
...
def check_file_path_hard_deny(
    ...
    :meth:`~toolguard.configuration.config.Configuration.hard_deny`). The path is hard-denied
    ...
    Args:
        context: Supplies ``tool_name`` (``'Read'``, ``'Write'``, or ``'Edit'``),
            ...
            :class:`~toolguard.decision_model.vocabulary.ResolveContext`.
        file_path: The file path under evaluation.

    Returns:
        A :class:`~toolguard.decision_model.vocabulary.LevelMatch` with
