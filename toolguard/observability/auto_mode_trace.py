...
from their base settings (see :mod:`toolguard.configuration.config`); this trace records
...
exactly as :mod:`toolguard.observability.log_writer`'s ``LogRecord`` does for the main
resolution log.
"""
...
def log_auto_mode_trace(entry: AutoModeTraceEntry, log_dir: Optional[Path]) -> None:
    ...
    (:mod:`toolguard.observability.error_log`), JSONL like :mod:`toolguard.observability.log_writer`'s
    ...
    :mod:`toolguard.observability.error_log`'s sibling streams.
