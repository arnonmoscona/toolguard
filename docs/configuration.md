...
**The effective group also decides how command prefixes are handled**, so a migrated rule can cover a slightly different set of commands in each mode -- an `allow` matches past only known-safe environment assignments, while `ask` and `deny` match past any. Nothing fails open, but a rule can quietly stop covering a command you expected. See [Auto-mode: `auto_mode_behavior` and command prefixes](auto-mode.md#auto_mode_behavior-and-command-prefixes) for which migrations are affected and when to reach for a `[regex]` pattern instead.
